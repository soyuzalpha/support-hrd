...
import { deleteCity, getCities, activateCity } from "./api/city-service";
import { createInputOptions, isEmpty, toCapitalized } from "@/utils";
import { useMutation } from "@tanstack/react-query";
import CardMaster from "@/components/CardMaster";
import { ConfirmationDialog } from "@/components/confirmation-dialog";
import { columnsMasterCity } from "./utils";
...
const City = () => {
  ...
  return (
    <FormProvider {...fForm}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            {company?.data?.data?.map((item, index) => (
              <CardMaster
                ...
                description={item.province?.province_name}
                item={item}
                onClickDetail={() => {
      ...
      <FormCity dialogHandler={dDialog} />
