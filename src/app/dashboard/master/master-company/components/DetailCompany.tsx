...
const DetailCompany = ({ dialogHandler }: { dialogHandler: UseDialogModalReturn }) => {
  ...
  return (
    <Dialog open={dialogHandler.open} onOpenChange={dialogHandler.handleClose}>
      <DialogContent size="xxl" glass={true}>
        <DialogHeader>
          <DialogTitle>{details.name_company}</DialogTitle>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h1 className="text-4xl font-bold text-white">{details.name_company}</h1>
                    <span
                  ...
                  <p className="text-white text-sm">Company ID: #{details?.id_company}</p>
                </div>
              </div>
              ...
              <GlassContainer>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white leading-relaxed">{details?.address_company}</p>
                  </div>
                </div>
              </GlassContainer>
            </div>
                  ...
                  <GlassContainer key={index} className="border-l-4 border-emerald-500 rounded-r-lg">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white">Log #{log.id_divisionlog}</span>
                      </div>
                    ...
                    {log?.action?.to && (
                      <div className="bg-white/40 rounded-lg p-4 space-y-2">
                        ...
                        <div className="text-sm space-y-1">
                          <p>
                            <span className="font-medium text-white">Position:</span> {log.action.to.name_division}
                          </p>
                          <p>
                            <span className="font-medium text-white">Description:</span>{" "}
                            {log.action.to.description_division}
                          </p>
                        </div>
                      </div>
                    )}
