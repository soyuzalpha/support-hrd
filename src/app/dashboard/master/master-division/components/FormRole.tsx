...
import { createDivisions, updateDivision } from "../api/master-position-service";
import { generateErrorMessage, generateSuccessMessage, isEmpty } from "@/utils";
import { toastAlert } from "@/lib/toast";
import { ToggleSwitch } from "@/components/ui/toggle-switch";
import * as z from "zod";
import { formSchemaDivision } from "../utils";
import { useAppRefreshQuery } from "@/hooks/use-refetch-data";
import Show from "@/components/show";

type FormSchemaType = z.infer<typeof formSchemaDivision>;
...
const FormRole = ({ dialogHandler }: { dialogHandler: UseDialogModalReturn }) => {
  ...
  const mutations = isEmpty(fForm.getValues("id_division")) ? mutation : updateMutation;

  const onSubmit: SubmitHandler<z.infer<typeof formSchemaDivision>> = (data) => {
    try {
      const payload = {
        id_division: data?.id_division,
        name_division: data?.name_division,
        description_division: data?.description_division,
  ...
  return (
    <Dialog open={dialogHandler.open} onOpenChange={dialogHandler.handleClose}>
      <DialogContent glass={true}>
        <DialogHeader>
          <DialogTitle>Form Division</DialogTitle>
        ...
        <form onSubmit={fForm.handleSubmit(onSubmit)}>
          <AppContainer fullWidth>
            <FieldGroup>
              ...
              <Show.When isTrue={isEmpty(fForm.getValues("id_division"))}>
                <FormField
                  control={fForm.control}
                  name="status"
                  render={({ field }) => (
