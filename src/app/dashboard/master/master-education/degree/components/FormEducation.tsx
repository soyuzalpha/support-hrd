...
const FormEducation = ({ dialogHandler }: { dialogHandler: UseDialogModalReturn }) => {
  ...
  const mutations = isEmpty(fForm.getValues("id_degree")) ? mutationCreate : mutationUpdate;

  const onSubmit = async (data) => {
    try {
      const payload = {
        id_degree: data?.id_degree,
        name_degree: data?.name_degree,
        ...
        <form onSubmit={fForm.handleSubmit(onSubmit)}>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Sarjana" {...field} aria-invalid={fieldState.invalid} />
                    </FormControl>
                    <FormDescription>Degree display name.</FormDescription>
          ...
          <DialogFooter className="mt-4">
            ...
            <Button type="submit" variant={"success"} loading={mutationCreate.isPending || mutationUpdate.isPending}>
              {fForm.getValues("id_degree") ? "Update" : "Create"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
